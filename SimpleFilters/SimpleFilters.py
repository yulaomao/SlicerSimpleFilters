...
class SimpleFiltersLogic:
  ...
  def run(self, filter, outputDMMLNode, outputLabelMap, *inputs):
    """
    ...
    self.output = None
    # check
    self.outputNodeName = outputDMMLNode.GetName()
...
class FilterParameters:
  ...
  def create(self, json):
    ...
    if json["template_code_filename"] == "KernelImageFilter":
      ...
      fiducialSelector = cjyx.qDMMLNodeComboBox()
      self.widgets.append(fiducialSelector)
      fiducialSelector.nodeTypes = ( "vtkDMMLMarkupsFiducialNode", "vtkDMMLAnnotationHierarchyNode")
      fiducialSelector.addAttribute("vtkDMMLAnnotationHierarchyNode", "MainChildType", "vtkDMMLAnnotationFiducialNode" )
      ...
      fiducialSelector.setDMMLScene( cjyx.dmmlScene )
      fiducialSelector.setToolTip( "Pick the Markups node for the seed list." )

      fiducialSelector.connect("nodeActivated(vtkDMMLNode*)", lambda node,name=name:self.onFiducialListNode(name,node))
      self.widgetConnections.append((fiducialSelector, "nodeActivated(vtkDMMLNode*)"))
    ...
    for member in json["members"]:
      ...
      if "dim_vec" in member and int(member["dim_vec"]):
        if member["itk_type"].endswith("IndexType") or member["itk_type"].endswith("PointType"):
          ...
          fiducialSelector = cjyx.qDMMLNodeComboBox()
          self.widgets.append(fiducialSelector)
          fiducialSelector.nodeTypes = (  "vtkDMMLMarkupsFiducialNode", "vtkDMMLAnnotationFiducialNode" )
          fiducialSelector.selectNodeUponCreation = True
          ...
          fiducialSelector.connect("nodeActivated(vtkDMMLNode*)", lambda node,w=fiducialSelector,name=member["name"],isPt=isPoint:self.onFiducialNode(name,w,isPt))
          self.widgetConnections.append((fiducialSelector, "nodeActivated(vtkDMMLNode*)"))
        ...
        fiducialSelector.connect("nodeActivated(vtkDMMLNode*)", lambda node,name=member["name"]:self.onFiducialListNode(name,node))
        self.widgetConnections.append((fiducialSelector, "nodeActivated(vtkDMMLNode*)"))
    ...
    self.outputSelector.nodeTypes = ["vtkDMMLScalarVolumeNode", "vtkDMMLLabelMapVolumeNode"]
    ...
    self.outputSelector.connect("nodeActivated(vtkDMMLNode*)", lambda node:self.onOutputSelect(node))
    self.widgetConnections.append((self.outputSelector, "nodeActivated(vtkDMMLNode*)"))
  ...
  def createInputWidget(self,n, noneEnabled=False):
      inputSelector = cjyx.qDMMLNodeComboBox()
      self.widgets.append(inputSelector)
      inputSelector.nodeTypes = ["vtkDMMLScalarVolumeNode", "vtkDMMLLabelMapVolumeNode"]
      ...
      inputSelector.setDMMLScene( cjyx.dmmlScene )
      inputSelector.setToolTip( "Pick the input to the algorithm." )

      # connect and verify parameters
      inputSelector.connect("nodeActivated(vtkDMMLNode*)", lambda node,i=n:self.onInputSelect(node,i))
      self.widgetConnections.append((inputSelector, "nodeActivated(vtkDMMLNode*)"))
  ...
  def onInputSelect(self, dmmlNode, n):
    self.inputs[n] = dmmlNode

  def onOutputSelect(self, dmmlNode):
    self.output = dmmlNode
    self.onOutputLabelMapChanged(dmmlNode.IsA("vtkDMMLLabelMapVolumeNode"))

  def onOutputLabelMapChanged(self, v):
    self.outputLabelMap = v
    self.outputLabelMapBox.setChecked(v)

  def onFiducialNode(self, name, dmmlWidget, isPoint):
    if not dmmlWidget.visible:
      return
    annotationFiducialNode = dmmlWidget.currentNode()

    # point in physical space
    coord = [0,0,0]

    if annotationFiducialNode.GetClassName() == "vtkDMMLMarkupsFiducialNode":
      # cjyx4 Markups node
  ...
  def onFiducialListNode(self, name, dmmlNode):
    annotationHierarchyNode = dmmlNode

    # list of points in physical space
    coords = []

    if annotationHierarchyNode.GetClassName() == "vtkDMMLMarkupsFiducialNode":
      # cjyx4 Markups node
